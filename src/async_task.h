...
public:
    ...
    {
        async_op<F> op(func);
        snode_core::instance().get_threadpool().schedule(op, id);
    }

    template <typename F, typename A1>
    static void inline connect(F func, A1 a1, thread_id_t id = THIS_THREAD_ID())
    {
        async_op<std::function<void()> > op(std::bind(func, a1));
        snode_core::instance().get_threadpool().schedule(op, id);
    }
    ...
    {
        async_op<std::function<void()> > op(std::bind(func, a1, a2));
        snode_core::instance().get_threadpool().schedule(op, id);
    }
    ...
    {
        async_op<std::function<void()> > op(std::bind(func, a1, a2, a3));
        snode_core::instance().get_threadpool().schedule(op, id);
    }
    ...
    {
        async_op<std::function<void()> > op(std::bind(func, a1, a2, a3, a4));
        snode_core::instance().get_threadpool().schedule(op, id);
    }
    ...
    {
        async_op<std::function<void()> > op(std::bind(func, a1, a2, a3, a4, a5));
        snode_core::instance().get_threadpool().schedule(op, id);
    }
};
